...
pub fn decode(src: &[u8], buf: &mut BytesMut) -> Result<BytesMut, CapybaraError> {
    ...
    if !decoder.is_final() {
        return Err(CapybaraError::InvalidHuffmanCode);
    }
...
impl Decoder {
    ...
    // Decodes 4 bits
    fn decode4(&mut self, input: u8) -> Result<Option<u8>, CapybaraError> {
...
mod test {
    ...
    fn decode(src: &[u8]) -> Result<BytesMut, CapybaraError> {
